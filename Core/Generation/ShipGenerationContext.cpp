...
{
    ShipGenerationContext::ShipGenerationContext(const ExplicitShipGenerationConfiguration& settings, const ShipGenerationProfile& profile, const ShipFactionProfile& factionProfile, const ShipGenerationSeeds& seeds, ShipGenerationDebugInfo* debugInfo, const GenerationCalibrationSettings* calibrationSettings)
        : Settings(settings), Profile(profile), FactionProfile(factionProfile), ScaleTraits(GenerationScaleTraits::fromDimensions(settings.Dimensions)), ComplexityBudget(GenerationComplexityBudget::create(ScaleTraits, profile, factionProfile)), Seeds(seeds), DomainSeeds(resolveGenerationDomainSeeds(seeds, settings.DomainSeedOverrides)), DebugInfo(debugInfo), CalibrationSettings(calibrationSettings), m_SavedCalibrationRandomGenerator(0u)
    {
    ...
    {
        return getRandomUInt(getDomainRandomGenerator(domain), minimum, maximum);
    }
    ...
    {
        return getRandomUInt64(getDomainRandomGenerator(domain), minimum, maximum);
    }
    ...
    void ShipGenerationContext::beginGenerationDomainCalibrationSubstream(GenerationDomain domain, uint64_t salt)
    {
        ...
        std::mt19937_64& generator = getDomainRandomGenerator(domain);
        m_SavedCalibrationRandomGenerator = generator;
        generator.seed(mixGenerationSeed64(DomainSeeds.get(domain) ^ salt));
    ...
    {
        if (!m_CalibrationSubstreamActive) { return; }
        getDomainRandomGenerator(m_CalibrationDomain) = m_SavedCalibrationRandomGenerator;
    ...
    {
        ComplexityBudget = GenerationComplexityBudget::create(ScaleTraits, Profile, FactionProfile);
